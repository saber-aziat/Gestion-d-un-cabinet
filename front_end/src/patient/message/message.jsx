...
const Messagee = () => {
    ...
    useEffect(() => {
        if (selectedConv) {
            fetchMessages(selectedConv.doctor_id || selectedConv.user_id);
        }
    }, [selectedConv]);
    ...
    const fetchConversations = async () => {
        ...
        try {
            ...
            const convs = followedDoctors.map(f => ({
                id: f.id,
                doctor_id: f.doctor_id || f.user_id,
                name: f.doctor_name || `${f.doctor_first_name} ${f.doctor_last_name}`,
                photo: f.doctor_photo,
            ...
            // If we came from the hospital/doctor page with a specific ID
            if (location.state?.selectedDoctorId) {
                const targetConv = convs.find(c => (c.doctor_id || c.user_id) === location.state.selectedDoctorId);
    ...
    const fetchMessages = async (doctorId) => {
        ...
        try {
            ...
            setMessages([
                { id: 1, sender_id: doctorId, text: "Bonjour, comment puis-je vous aider aujourd'hui ?", created_at: "10:30" },
                { id: 2, sender_id: user.id, text: "Bonjour Docteur, j'ai une question sur mon traitement.", created_at: "10:35" },
            ]);
        } finally {
    ...
    const handleSendMessage = async (e) => {
        ...
        const messageData = {
            receiver_id: selectedConv.doctor_id || selectedConv.user_id,
                ...
                {selectedConv ? (
                    ...
                    <div className={styles.emptyChat}>
                        ...
                        >
                            ...
                            <p>Sélectionnez un médecin à gauche pour commencer à discuter.</p>
                        </motion.div>
                    </div>
                )}
            </main>
